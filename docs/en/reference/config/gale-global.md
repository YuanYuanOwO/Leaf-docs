...
5. Whether to add a [NOT SECURE] marker in front unsigned chat packets
...
9. When a very old Bukkit plugins is loaded
...
23. The interval, given in ticks, at which to increase the time-related statistics such as total playtime, time since the last death, etc.<br>
  ...
  <br>
  If a value &leq; <code>0</code> is given, it will default to the same as Paper.
